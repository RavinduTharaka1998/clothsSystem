...
$result = filter_Table($sql);

function filter_Table($sql)
{
	$connection = mysqli_connect("localhost:3307","root","","cloths_db");
...
<head>
    ...
    <!--stylesheet link-->
    <link rel="stylesheet" href="css/mydress.css">
...
<body>
            ...
            <div class = "top-middle">
                ...
                <div class = "top-middle-right">
                    <a href="#"></i>My Dress <i class="fas fa-chevron-right"></i></a>
                </div>
                <div class = "profile">
                    ...
                    <table>
                        ...
                        <?php
                                ...
                                while($row = mysqli_fetch_array($result))
                                {
                                    ...
                                    <tr>
                                        <td><?php echo "<p>".$row['cloth_id']."</p>";?></td>
                                        <td><?php echo "<p>".$row['description']."</p>";?></td>
                                        <td><?php echo "<p>".$row['price']."</p>";?></td>
                                        <td><?php echo "<p>".$row['date']."</p>";?></td>
                                        <td><?php echo "<p>".$row['address']."</p>";?></td>
                                        <td>
                                            <form method="post" action="editcloth.php">
                                                <input type="hidden" name="id" value="<?php echo $row['order_id']; ?>"/>
                                                <input type="submit" class="update" name="editcloth" value="Edit"/>
                                            </form>
                                        </td>
                                        <td><?php echo "<button type='submit' onclick='delete_cloth($id)' class='delete'>Delete</button>";?></td>
                                    </tr>
                                    <?php
    ...
    <script>
		...
		{
			if(confirm("Are you sure to delete your cloth order ?"))
			{
				window.location.href="backend/deletecloth.php?id="+id;
			}
		}
    </script>
</body>
